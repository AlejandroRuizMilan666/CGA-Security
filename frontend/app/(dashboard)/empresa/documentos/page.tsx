...
export default function EmpresaDocumentosPage() {
  ...
  async function handleUpload() {
    ...
    setError(null);
    try {
      // Empresa siempre sube documentos entrantes (desde su perspectiva son salientes hacia CGA)
      const direction: DocumentDirection = 'OUTGOING';
      await uploadDocument({ file, direction, notes: notes || undefined });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
        <h2 className="mb-4 text-sm font-semibold text-white">Enviar documento a CGA Security</h2>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <DocumentTable
              ...
              emptyText="No has recibido documentos todavía"
            />
          </div>
          ...
          <div>
            ...
            <DocumentTable
              ...
              onDelete={handleDelete}
              deletingId={deletingId}
              emptyText="No has enviado documentos todavía"
            />
          </div>
        </div>
      )}
        ...
        <tbody>
          {documents.map((doc) => (
            <tr key={doc.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
              ...
              <td className="px-4 py-3">
                <Badge
                  label={doc.direction === 'INCOMING' ? 'Recibido' : 'Enviado'}
                  variant={doc.direction === 'INCOMING' ? 'purple' : 'cyan'}
                />
              </td>
